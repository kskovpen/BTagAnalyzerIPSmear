  ...
  m_config(iConfig),m_helper(iConfig,consumesCollector())
{
  ...
  m_isData                  = m_config.getParameter<bool>("isData");
  m_applySmearing           = m_config.getParameter<bool>("applySmearing");
  ...
  ipsmear = new IPSmear;
  ipsmear->init(m_isData,m_applySmearing);
}
...
*/
	...
	std::vector<double> res = ipsmear->smear(track.normalizedChi2(),track.eta(),track.hitPattern().numberOfValidPixelHits(),track.p(),
						 track.pt(),track.hitPattern().numberOfValidStripHits(),track.dxy(pv->position()),
						 track.dz(pv->position()),trackIP.distanceToJetAxis.value(),decayLength,dxyErr,dzErr);
	...
	float sign2D = (trackIP.ip2d.value() > 0) ? 1 : -1;
	float sign3D = (trackIP.ip3d.value() > 0) ? 1 : -1;
	
	double ip2dSigSmear = sign2D*fabs(dxy)/dxyErr;
	double ip3dSigSmear = sign3D*sqrt(pow(dxy/dxyErr,2)+pow(dz/dzErr,2));
	
	trackIP.ip2d = Measurement1D(ip2dSigSmear,1.);
	trackIP.ip3d = Measurement1D(ip3dSigSmear,1.);
...
void IPProducer<reco::TrackRefVector, reco::JTATagInfo, IPProducerHelpers::FromJTA>::fillDescriptions(edm::ConfigurationDescriptions & descriptions) {
  ...
  desc.add<bool>("applySmearing",false);
